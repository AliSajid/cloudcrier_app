...
```bash
cloudcrier-app$ sam build --use-container
```
...
```bash
cloudcrier-app$ sam logs -n StockCheckerFunction --stack-name "cloudcrier-app" --tail
```
...
```bash
cloudcrier-app$ pip install -r tests/requirements.txt --user
# unit test
cloudcrier-app$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
cloudcrier-app$ AWS_SAM_STACK_NAME="cloudcrier-app" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "cloudcrier-app"
```
